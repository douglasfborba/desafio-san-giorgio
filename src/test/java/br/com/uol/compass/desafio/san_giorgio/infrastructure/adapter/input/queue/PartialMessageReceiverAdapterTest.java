...
class PartialMessageReceiverAdapterTest {
    ...
    @InjectMocks
    private PartialMessageReceiverAdapter partialMessageReceiver;

    @Test
    @DisplayName("Should receive payment message successfully.")
    void shouldReceivePaymentMessageSuccessfully() throws Exception {
        var message = PaymentMessageFactory.createPartialPaymentMessage();
        ...
        when(updatePaymentStatusUseCase.update(any(PaymentStatus.class), any(Payment.class)))
                .thenReturn(PaymentFactory.createPartialPayment());

        partialMessageReceiver.receive(payload);
    ...
    void shouldThrowReceiveMessageErrorExceptionWhenCatchUnexpectedError() throws Exception {
        ...
        assertThat(exception.getMessage()).contains("Error to receive message from the queue.");
    }
}
